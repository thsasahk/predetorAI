...
public class DgGenerator : MonoBehaviour
{
    ...
    */
    ...
    void FillRoom(DgDivision.DgRect r)
    {
    ...
    bool CreateRoad(DgDivision divA, DgDivision divB)
    {
        if (divA.Outer.Bottom == divB.Outer.Top || divA.Outer.Top == divB.Outer.Bottom)
        {
            ...
            if (divA.Outer.Top > divB.Outer.Top)
            {
                ...
                y = divA.Outer.Top;
                // 通路を作成
                _layer.FillRectLTRB(x1, y + 1, x1 + 1, divA.Room.Top, CHIP_NONE);
                _layer.FillRectLTRB(x2, divB.Room.Bottom, x2 + 1, y, CHIP_NONE);
            }
            else
            {
                // A - B (Aが上側)
                y = divB.Outer.Top;
                // 通路を作成
                _layer.FillRectLTRB(x1, divA.Room.Bottom, x1 + 1, y, CHIP_NONE);
                _layer.FillRectLTRB(x2, y, x2 + 1, divB.Room.Top, CHIP_NONE);
            }
        ...
        if (divA.Outer.Left == divB.Outer.Right || divA.Outer.Right == divB.Outer.Left)
        {
            ...
            if (divA.Outer.Left > divB.Outer.Left)
            {
                ...
                x = divA.Outer.Left;
                // 通路を作成
                _layer.FillRectLTRB(divB.Room.Right, y2, x, y2 + 1, CHIP_NONE);
                _layer.FillRectLTRB(x + 1, y1, divA.Room.Left, y1 + 1, CHIP_NONE);
            }
            else
            {
                // A - B (Aが左側)
                x = divB.Outer.Left;
                _layer.FillRectLTRB(divA.Room.Right, y1, x, y1 + 1, CHIP_NONE);
                _layer.FillRectLTRB(x, y2, divB.Room.Left, y2 + 1, CHIP_NONE);
            }
